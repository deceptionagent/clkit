...
Parameter options can be required, meaning they must be present in the argument vector or CLKArgumentParser
will automatically error.

A recurrent option may be provided more than once. Switch options are always recurrent. Parameter options
are non-recurrent by default.

Options can have a list of dependencies, which are parameter options that are required when a particular
option is provided. (E.g., "--foo requires --bar, but --bar is not required on its own.")
...
// [TACK] this needs a clear, concise demonstration
